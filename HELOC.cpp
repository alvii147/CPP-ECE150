...
int processInputs(const int argc, const char* const argv[], int&   lineOfCreditMaximum, float& interestRate, int&   initialBalance, int&   numberOfYears, int*&  payments)
{
	int rc = 0;
...
float helocFinalBalance(const int lineOfCreditMaximum, const float interestRate, const int initialBalance, const int numberOfYears, const int payments[])
{
	if(lineOfCreditMaximum<0 || interestRate<0 || numberOfYears<1)
	{
		return NAN;
	}
	...
	while(i<numberOfMonths)
	{
		if(finalBalance>0)
		{
			interest = ((interestRate/12.0)/100.0)*finalBalance;
		}
		else
		{
			interest = 0;
		}
		finalBalance += interest-payments[i];
		if(finalBalance>lineOfCreditMaximum)
		{
			finalBalance += (0.01*finalBalance);
		}
		i++;
	}
	return finalBalance;
...
float totalInterestPaid(const int lineOfCreditMaximum, const float interestRate, const int initialBalance, const int numberOfYears, const int payments[])
{
	if(lineOfCreditMaximum<0 || interestRate<0 || numberOfYears<1 || initialBalance>lineOfCreditMaximum)
	{
		return NAN;
	}
	float finalBalance = initialBalance;
